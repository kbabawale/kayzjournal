...
if(isset($_POST['submit'])){
	...
	$expected = array('name_of_activity');
...
if(!$suspect && !$missing){
		...
		$sql = 'INSERT INTO activities (name_of_activity, date_created) VALUES (?,NOW())';
			
			//require_once('includes/connection.inc.php');

			$stmt = $conn->stmt_init();
			$stmt->prepare($sql);
			$stmt->bind_param('s', $name_of_activity);
			$stmt->execute();
			$err = $stmt->error;
			
			if($stmt->affected_rows == 1){
				header('Location:personal_philosophy.php?ref=activities');
				exit;	
			}elseif($err){
			}
...
<head>
<meta charset="utf-8">
<title>Add new activity - Kayz Journal</title>
                        	...
                        	<table border="0" width="100%" style="font-style:normal;">
                                <tr>
                                    ...
                                    <td  width="50%"> 
                                        ...
                                        <?php if($missing || $errors) {echo 'value="'. htmlentities($_POST['name_of_activity'], ENT_COMPAT, 'UTF-8') 
										.'"';} ?>
                                ...
                                <tr>
                                	<td>
                                        ...
                                        onMouseOut="this.style.background='black'" value="Add Activity">
                                    </td>
                                </tr>
