...
if(!$suspect && !$missing){
		...
		$sql = 'INSERT INTO quotes (quote_details, date_created, quoter) VALUES (?,NOW(),?)';
			
			//require_once('includes/connection.inc.php');

			$stmt = $conn->stmt_init();
			$stmt->prepare($sql);
			$stmt->bind_param('ss', $quote, $quoter);
			$stmt->execute();
			$err = $stmt->error;
			
			if($stmt->affected_rows == 1){ 
				header('Location:personal_philosophy.php?ref=quotes');
				exit;	
			}elseif($err){
			}
...
<head>
<meta charset="utf-8">
<title>Add quote - Kayz Journal</title>
                        ...
                        <form method="post" action="" id="frm1" name="frm1">
                        	<table border="0" width="100%" style="font-style:normal;">
                                ...
                                <tr>
                                    ...
                                    <td  width="50%"> 
                                        <input type="text" id="quoter" name="quoter"
                                        <?php if($missing || $errors) {echo 'value="'. htmlentities($_POST['quoter'], ENT_COMPAT, 'UTF-8') 
										.'"';} ?>
                                ...
                                <tr>
                                	<td>
                                        ...
                                        onMouseOut="this.style.background='black'" value="Add Quote">
                                    </td>
                                </tr>
