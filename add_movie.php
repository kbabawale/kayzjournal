...
if(isset($_POST['submit'])){
	...
	$expected = array('name_of_movie','year','genre','edit_status');
...
if(!$suspect && !$missing){
		...
		$sql = 'INSERT INTO movies (name_of_movie, year, genre, status_id, date_created) VALUES (?,?,?,?,NOW())';
			
			//require_once('includes/connection.inc.php');

			$stmt = $conn->stmt_init();
			$stmt->prepare($sql);
			$stmt->bind_param('sisi', $name_of_movie, $year, $genre, $edit_status);
			$stmt->execute();
			$err = $stmt->error;
			
			if($stmt->affected_rows == 1){
				header('Location:personal_philosophy.php?ref=movies');
				exit;	
			}elseif($err){
			}
...
<head>
<meta charset="utf-8">
<title>Add new movie - Kayz Journal</title>
                        ...
                        <form method="post" action="" id="frm1" name="frm1">
                        	<table border="0" width="100%" style="font-style:normal;">
                                <tr>
                                    ...
                                    <td  width="50%"> 
                                        <input type="text" id="name_of_movie" name="name_of_movie"
                                        <?php if($missing || $errors) {echo 'value="'. htmlentities($_POST['name_of_movie'], ENT_COMPAT, 'UTF-8') 
										.'"';} ?>
                                ...
                                <tr>
                                    ...
                                    <td  width="50%"> 
                                        ...
                                        <?php if($missing || $errors) {echo 'value="'. htmlentities($_POST['genre'], ENT_COMPAT, 'UTF-8') 
										.'"';} ?>
                                        style="width:80%; height:30px; margin:5px 0 0 0; padding:5px;color:#000; background:inherit; 
                                        border:#000 solid 1px;
                                        font-size:1.0em;font-family:'Microsoft JhengHei',Microsoft JhengHei Light;"/>
                                ...
                                <tr>
                                	<td>
                                        ...
                                        onMouseOut="this.style.background='black'" value="Add Movie">
                                    </td>
                                </tr>
